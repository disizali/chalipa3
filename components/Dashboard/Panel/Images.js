...
export class Images extends Component {
  ...
  componentDidMount() {
    axios.get("http://http://95.216.86.208:3000/api/images").then(({ data: images }) => {
      this.setState({
        images
      });
    });
  ...
  deleteImage(targetFileName) {
    axios
      .delete("http://http://95.216.86.208:3000/api/images", { data: { targetFileName } })
      .then(({ data }) => {
        this.setState({
  ...
  uploadImage() {
    ...
    axios.post("http://http://95.216.86.208:3001/api/upload", data).then(({ data }) => {
    ...
    return (
      <div>
        <Container className="p-5 d-flex flex-column">
          ...
          <Form action="http://http://95.216.86.208:3001/api/upload" method="POST">
            <FormGroup>
              <Label
