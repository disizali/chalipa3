...
export class Images extends Component {
  ...
  deleteImage(targetFileName) {
    axios
      .delete("http://localhost/api/images", { data: { targetFileName } })
      .then(({ data }) => {
        this.setState({
  ...
  uploadImage() {
    ...
    axios.post("http://localhost:3000/api/upload", data).then(({ data }) => {
    ...
    return (
      <div>
        <Container className="p-5 d-flex flex-column">
          ...
          <Form action="http://localhost:3000/api/upload" method="POST">
            <FormGroup>
              <Label
