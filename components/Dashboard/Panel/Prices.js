...
export class Prices extends Component {
  ...
  componentDidMount() {
    axios.get("http://95.216.86.208/api/prices").then(({ data: prices }) => {
  ...
  sendPrice() {
    ...
    axios
      .post("http://95.216.86.208/api/prices", { code, price, size })
      .then(({ data }) => {
        if (data != "error") {
          const newPrices = [
  ...
  deletePrice(targetId) {
    axios
      .delete("http://95.216.86.208/api/prices", { data: { targetId } })
      .then(({ data }) => {
        if (data != "no price") {
          this.setState({
